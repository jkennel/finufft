...
// Author: Andrea Malleo, 2019.
{
  ...
  if (ier>1) {   // since 1 (a warning) still allows proceeding...
    REprintf("Error", "FINUFFT invokeGuru: plan error (ier=%d)!\n", ier);
    return ier;
  }

  int ier2 = FINUFFT_SETPTS(plan, nj, xj, yj, zj, nk, s, t, u);
  if (ier2>1) {
    REprintf("Error","FINUFFT invokeGuru: setpts error (ier=%d)!\n", ier2);
    return ier2;
  }

  int ier3 = FINUFFT_EXECUTE(plan, cj, fk);
  if (ier3>1) {
    REprintf("Error","FINUFFT invokeGuru: execute error (ier=%d)!\n", ier3);
    return ier3;
  }
